...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests() // 認可に関する設定
				// すべてのユーザがアクセスできるリンク
				.antMatchers("/", "/registration/**", "/toLogin", "/findName", "/item/**", "/cart/**", "/common/**", "/error").permitAll()
		...
		http.formLogin() // ログインに関する設定
				...
				.defaultSuccessUrl("/?login", false) // 第1引数:デフォルトでログイン成功時に遷移させるパス
		...
		http.logout() // ログアウトに関する設定
				...
				.logoutSuccessUrl("/?logout") // ログアウト後に遷移させるパス(ここではログイン画面を設定)
	...
	/**
	 * ログイン成功した時の処理. 実装する際は .formLogin().successHandler(new
	 ...
	 * IOException, ServletException { System.out.println("ログイン成功");
	 * 
	 * } }
	 */
